...
internal class HeaderPositionCalculator<VH : RecyclerView.ViewHolder>(
    ...
    private fun initDefaultHeaderOffset(headerMargins: Rect, recyclerView: RecyclerView, header: View, firstView: View, orientation: Int) {
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            translationY = Math.max(firstView.top - topMargin - header.height - tempRect1.bottom,
                    getListTop(recyclerView) + tempRect1.top)
        } else {
            translationY = firstView.top - topMargin + tempRect1.top
            translationX = Math.max(firstView.left - leftMargin - header.width - tempRect1.right,
        ...
        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(firstViewUnderHeaderPosition, isReverseLayout)) {
            ...
            if (viewAfterHeader != null) {
                return if (orientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                    val topOfNextHeader: Int = viewAfterHeader.top - tempRect1.bottom - nextHeader.height - tempRect1.top
                    val bottomOfThisHeader: Int = recyclerView.paddingTop + stickyHeader.bottom + tempRect2.top + tempRect2.bottom
                    topOfNextHeader < bottomOfThisHeader
                } else {
                    val leftOfNextHeader: Int = viewAfterHeader.left - tempRect1.right - nextHeader.width - tempRect1.left
                    val rightOfThisHeader: Int = recyclerView.paddingLeft + stickyHeader.right + tempRect2.left + tempRect2.right
    ...
    private fun translateHeaderWithNextHeader(recyclerView: RecyclerView, orientation: Int, translation: Rect,
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            val topOfStickyHeader: Int = getListTop(recyclerView) + tempRect2.top + tempRect2.bottom
            val shiftFromNextHeader: Int = (viewAfterNextHeader?.top ?: 0) - nextHeader.height -
                    tempRect1.bottom - tempRect1.top - currentHeader.height - topOfStickyHeader
            if (shiftFromNextHeader < topOfStickyHeader) {
                translation.top += shiftFromNextHeader
            }
        } else {
            val leftOfStickyHeader: Int = getListLeft(recyclerView) + tempRect2.left + tempRect2.right
            val shiftFromNextHeader: Int = (viewAfterNextHeader?.left ?: 0) - nextHeader.width -
                    tempRect1.right - tempRect1.left - currentHeader.width - leftOfStickyHeader
    ...
    private fun itemIsObscuredByHeader(parent: RecyclerView, item: View, header: View, orientation: Int): Boolean {
        ...
        return if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            val headerBottom: Int = getListTop(parent) + header.bottom + tempRect1.bottom + tempRect1.top
            itemTop < headerBottom
        } else {
            val itemLeft: Int = item.left - layoutParams.leftMargin
            val headerRight: Int = getListLeft(parent) + header.right + tempRect1.right + tempRect1.left
