...
class SideAndStickyHeaderRecyclerView : FrameLayout {
     ...
     */
    private var decoration: StickyRecyclerHeadersDecoration<*>? = null
    /**
     * 滑动监听，联动wave side
     */
     ...
     *
     * @param adapter [StickyHeaderBindAdapter]
     */
    fun <T, SB : ViewDataBinding, B : ViewDataBinding> setAdapter(adapter: StickyHeaderBindAdapter<T, SB, B>) {
    ...
    private fun setLazyRespond(lazyRespond: Boolean) = sideBar.setLazyRespond(lazyRespond)

    fun setIndexItems(vararg indexItems: String) = sideBar.setIndexItems(Arrays.copyOf(indexItems, indexItems.size))

    fun setIndexItems(indexItems: List<String>) = sideBar.setIndexItems(indexItems.toTypedArray())

    fun setTextColor(@ColorInt color: Int) = sideBar.setTextColor(color)

    fun setPosition(position: Int) = sideBar.setPosition(position)

    fun setMaxOffset(offset: Int) = sideBar.setMaxOffset(offset)

    fun setTextAlign(align: Int) = sideBar.setTextAlign(align)
