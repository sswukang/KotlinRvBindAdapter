...
class StickyRecyclerHeadersDecoration<VH : RecyclerView.ViewHolder> private constructor(
    ...
    private val headerRectArray = SparseArray<Rect>()
    ...
    override fun onDrawOver(canvas: Canvas, parent: RecyclerView, state: RecyclerView.State?) {
        ...
        for (i: Int in 0 until childCount) {
            ...
            if (hasStickyHeader || headerPositionCalculator.hasNewHeader(position, orientationProvider.isReverseLayout(parent))) {
                ...
                var headerOffset: Rect? = headerRectArray.get(position)
                if (headerOffset == null) {
                    headerOffset = Rect()
                    headerRectArray.put(position, headerOffset)
                }
    ...
    fun findHeaderPositionUnder(x: Int, y: Int): Int {
        for (i: Int in 0 until headerRectArray.size()) {
            val rect: Rect = headerRectArray.get(headerRectArray.keyAt(i))
            if (rect.contains(x, y)) {
                val position: Int = headerRectArray.keyAt(i)
