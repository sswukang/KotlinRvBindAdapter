...
class StickyRecyclerHeadersTouchListener<VH : RecyclerView.ViewHolder>(
    ...
    private val tapDetector: GestureDetector by lazy { GestureDetector(recyclerView.context, SingleTapDetector()) }
    ...
    override fun onInterceptTouchEvent(view: RecyclerView, e: MotionEvent): Boolean {
        if (this.onHeaderClickListener != null) {
            val tapDetectorResponse: Boolean = this.tapDetector.onTouchEvent(e)
            if (tapDetectorResponse) {
                // Don't return false if a single tap is detected
                return true
            }
            if (e.action == MotionEvent.ACTION_DOWN) {
                val position: Int = decoration.findHeaderPositionUnder(e.x.toInt(), e.y.toInt())
                return position != -1
            }
        }
        return false
    ...
    private inner class SingleTapDetector : GestureDetector.SimpleOnGestureListener() {
        override fun onSingleTapUp(e: MotionEvent): Boolean {
            val position: Int = decoration.findHeaderPositionUnder(e.x.toInt(), e.y.toInt())
            if (position != -1) {
                val headerView: View = decoration.getHeaderView(recyclerView, position)
                val headerId: Long = getAdapter().getHeaderId(position)
